...
from .helpers import fetch_available_crypto_tokens, Currency
import logging
...
class CryptoWalletConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_token_amounts(self, user_input=None):
        ...
        token_amounts_schema = {vol.Required(token): cv.positive_float for token in tokens}

        return self.async_show_form(step_id="token_amounts", data_schema=vol.Schema(token_amounts_schema))
...
class CryptoWalletOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        tokens = self.config_data.get(CONF_CRYPTO_TOKEN, [])
        _LOGGER.debug(f"async_step_init: before configured tokens: {tokens}")
        ...
        options_schema = vol.Schema({
            vol.Optional(CONF_CRYPTO_API_ACCESS_TOKEN, default=access_token): cv.string,
            vol.Optional(CONF_BASE_CURRENCY, default=selected_currency): selector.SelectSelector(
                selector.SelectSelectorConfig(
                    options=Currency.get_all_currency_codes(),
                    multiple=False,
                    mode=selector.SelectSelectorMode.DROPDOWN,
            ...
            vol.Optional(CONF_CRYPTO_TOKEN, default=tokens): selector.SelectSelector(
                selector.SelectSelectorConfig(
                    options=self.available_tokens,
                    multiple=True,
                    mode=selector.SelectSelectorMode.DROPDOWN,
    ...
    async def async_step_token_amounts(self, user_input=None):
        ...
        if user_input is not None:
            # Update token amounts in the config_data
            _LOGGER.debug(f"async_step_token_amounts: Updating config data with: {user_input}")
            self.config_data.update({CONF_TOKEN_AMOUNTS: user_input})
            self.hass.config_entries.async_update_entry(self.config_entry,
                                                        data=self.config_data)  # Update the entry with new data
            return self.async_create_entry(title="Crypto Wallet", data=self.config_data)

        tokens = self.config_data.get(CONF_CRYPTO_TOKEN, [])
        token_amounts = self.config_entry.data.get(CONF_TOKEN_AMOUNTS, {})
        token_amounts_schema = {
            vol.Required(token, default=token_amounts.get(token, 1.0)): vol.All(vol.Coerce(float), vol.Range(min=0)) for
            token in tokens}
